...
A_TYPE *sem_expression(A_NODE *node)
{
	...
	A_NODE *convertUsualAssignmentConversion(A_TYPE *t1, A_TYPE *node) {
		A_TYPE *t2;
		t2 = node->type;
	...
	A_NODE *convertUsualUnaryConversion(A_NODE *node) {
		...
		if (t == char_type) {
			...
			t->size = 4;
			node = makeNode(N_EXP_CAST, NIL, node);
			node->type = t;
		}
	...
	A_LITERAL checkTypeAndConversionLiteral(A_LITERAL result, A_TYPE *t, int ll) {
